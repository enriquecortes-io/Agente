...
function construirRespuesta(propiedades: any[], nombreCliente: string | null, zona: string | null): string {
 ...
 return intro + lista + '\n\n¿Te gustaría programar una visita privada o necesitas más información?';
}
...
export async function POST(req: Request) {
 ...
 try {
   ...
   // Búsqueda directa solo para compradores con zona o presupuesto
  ...
  const res = await fetch('https://integrate.api.nvidia.com/v1/chat/completions', {
     ...
     body: JSON.stringify({
       ...
       tools: [
         ...
         { type: 'function', function: { name: 'buscarPropiedades', description: 'Busca propiedades. Solo para Venta.', parameters: { type: 'object', properties: { zona: { type: 'string' }, precioMax: { type: 'number' } } } } },
       ],
       tool_choice: 'auto',
       max_tokens: 600,
       temperature: 0.4,
