...
function getDocIdParaTipo(tipoLead: TipoLead): string {
  ...
  const docId = map[tipoLead];
  if (!docId) throw new Error(`Falta GOOGLE_DOC_${tipoLead.toUpperCase()} en .env.local`);
  return docId;
}

/**
 * Registra al cliente en el doc maestro correspondiente al tipo de lead.
 * Inserta una cabecera de nuevo cliente con timestamp.
 * Devuelve el docId para usarlo en actualizarHistorial.
 */
export async function prepararEntornoCliente(nombreCliente: string, tipoLead: TipoLead) {
  ...
  console.log(`[Docs] Cliente ${nombreCliente} registrado en doc ${tipoLead}`);
